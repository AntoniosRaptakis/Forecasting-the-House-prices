...
Before I start with the cleaning part of the project, I must articulate that the I there are two jupyter notebooks for this project. One is referred to the description that I follow below, the **Predicting_the_house_prices_initial.ipynb**, and the other to a different data cleaning, the **Predicting_the_house_prices_different_cleaning.ipynb**. For the second one, I replaced some of the Nan values of the dataframe because based on the <ins>**data_description.txt**<ins> file (you can find it in the files_with_data directory) the <ins>NA</ins> had a specific meaning. Thus, it is interesting to give an eye to that one as well!
